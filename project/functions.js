var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	*/
	...
	case 27: // ESC：打开菜单栏
		...
		break;
	case 88: // X：纸条
		if (core.hasItem('I321')) {
			if (core.canUseItem('I321')) {
				core.status.route.push("key:88"); // 将按键记在录像中
				core.useItem('I321', true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
			} else {}
		}
		break;
	...
	case 75:
	case 86: // V：
		if (core.hasItem('redJewel')) {
			if (core.canUseItem('redJewel')) {
				core.status.route.push("key:86"); // 将按键记在录像中
				core.useItem('redJewel', true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
			} else {}
		}
		break;
	case 32: // SPACE：轻按
	...
	case 67: // C：左边老人
		if (core.hasItem('coin')) {
			if (core.canUseItem('coin')) {
				core.status.route.push("key:67"); // 将按键记在录像中
				core.useItem('coin', true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
			} else {}
		}
		break;
    ...
    "control": {
        "saveData": function () {
	if (core.status.floorId == "m50" || core.status.floorId == "m51") {
		core.drawTip("摸牌战中不能存档！");
	} else {
		var values = {};
	...
	if (core.status.floorId) {
		core.setStatusBarInnerHTML('hp', core.status.maps[core.status.floorId].name);
	}
	core.setStatusBarInnerHTML('name', core.getStatus('name'));
	core.setStatusBarInnerHTML('lv', core.getLvName());
	var statusList = ['mdef', 'money', 'experience'];
